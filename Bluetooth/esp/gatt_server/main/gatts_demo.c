...
RTC_DATA_ATTR uint16_t msg_id;
RTC_DATA_ATTR uint8_t transport_layer;
RTC_DATA_ATTR uint8_t protocol_id;
...
void manejo_read_event(esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param){
    ...
    if (transport_layer != 5 && protocol_id != 5 ) {
        ...
        esp_err_t err = esp_ble_gatts_send_response(gatts_if, param->read.conn_id, param->read.trans_id, ESP_GATT_OK, &resp);
        if (err != ESP_OK) {
            ESP_LOGE(TAG, "Error al enviar respuesta: %s", esp_err_to_name(err));
            return;
        }
        free(packet);
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_WRITE_EVT: {
        ...
        if (!param->write.is_prep){
            ...
            {
                ESP_LOGI(TAG, "LLego mensaje final");
...
{
    if (!initialized) {
        ESP_LOGI("RTC_MEMORY", "Inicializando variables RTC...");
        msg_id = 5;
        transport_layer = 5;
        protocol_id = 5;
        initialized = true; // Marcar como inicializado
    } else {
        ESP_LOGI("RTC_MEMORY", "Variables RTC ya inicializadas.");
    }
